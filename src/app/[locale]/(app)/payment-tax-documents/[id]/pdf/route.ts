...
export async function GET(
  ...
  if (!doc || doc.type !== "PAYMENT_TAX_DOCUMENT" || doc.deletedAt) notFound();
      ...
      try {
        ...
        return new Response(new Uint8Array(buffer), {
          headers: {
            ...
            "content-disposition": `${disp}; filename="${doc.number ?? "danovy-doklad"}.pdf"`,
  ...
  const fallback = `danovy-doklad-${doc.issueDate.toISOString().slice(0, 10)}`;
  return new Response(new Uint8Array(buffer), {
    headers: {
      "content-type": "application/pdf",
