...
export async function createCalendarEvent(
  ...
  const ev = await prisma.calendarEvent.create({
    data: { ...toPayload(parsed.data), createdById: user.id, workspaceId: workspace.id },
  });
  await writeAudit({
...
export async function updateCalendarEvent(
  ...
  const parsed = schema.safeParse(Object.fromEntries(formData));
  if (!parsed.success) return { error: "invalidInput" };
  const before = await prisma.calendarEvent.findFirst({ where: { id, workspaceId: workspace.id } });
...
export async function toggleComplete(id: string) {
  const { user, workspace } = await requireWorkspace();
  const ev = await prisma.calendarEvent.findFirst({ where: { id, workspaceId: workspace.id } });
  if (!ev) return;
